...
*)
let fib2 n =
  let rec aux n = (* fib n, fib n-1 *)
    if n<=0 then (1,0)
    else 
      let (a,b)=aux(n-1) (* fib n-1, fib n-2 *) 
      in (a+b,a) (* fib n, fib n-1 *)
  in fst(aux n);; 
...
let fib3 (n:int) : Num.num =
  let rec aux n =
    if n<=0 then (num_of_int 1,Num.num_of_int 0)
    else let (a,b)=aux(n-1) in (Num.(+/) a  b,a)
  in fst(aux n);; 
...
let factlist2 n =
  let rec aux n =  (* fact n, factlist n-1 *)
    if n=1 then (1,[])
    else 
      let (a,b) = aux (n-1)  (* fact n-1, factlist n-2 *)
      in (n*a, a::b)
  in snd (aux (n+1));;
  (* if n=0 then [] *)
  (* else let (a,b)=aux n in a::b *)
...
let choose_max x y =
  ...
  let s2 = sum y in
  if s1>s2 then x
  else if s1<s2 then y
  else if List.length x < List.length y then x
  else y

let rec find_max rs = match rs with
  | [a] -> a
  | b::bs -> choose_max b (find_max bs)

let max_prefix xs = 
  let rs = enum_prefix xs in
  let ans = find_max rs in
  ans
